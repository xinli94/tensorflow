...
int main(int argc, char* argv[]) {
  ...
  float input_mean = 0;
  float input_std = 255;
  string input_layer = "image_data";
  string output_layer = "labels_softmax";
  bool self_test = false;
  string root_dir = "";
  std::vector<Flag> flag_list = {
      Flag("image", &image, "image to be processed"),
      Flag("image_dir", &image_dir, "image dir to be processed"),
  ...
  for (int64_t i = 0; i < images.size(); i++) {
    ...
    string image_path = tensorflow::io::JoinPath(root_dir, images[i]);
    std::cout << "\n";
